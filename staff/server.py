...
class GuildActionsView(discord.ui.View):
    ...
    def _add_dynamic_buttons(self):
        ...
        if not hasattr(self.guild, '_cached_data'):  # Seulement si le bot est présent
            invite_btn = discord.ui.Button(
                label="Invite",
                emoji="<:link:1401600607949422683>",
                style=discord.ButtonStyle.success,
                row=1
            )
            invite_btn.callback = self.generate_invite
    ...
    async def generate_invite(self, interaction: discord.Interaction):
        """Génère une invitation pour le serveur"""
        if hasattr(self.guild, '_cached_data'):
            await interaction.response.send_message(
                "<:undone:1398729502028333218> Cannot create an invite for a server the bot is not in.",
                ephemeral=True
            )
            return
        ...
        if not real_guild:
            await interaction.response.send_message(
                "<:undone:1398729502028333218> Bot is no longer in this server.",
                ephemeral=True
            )
            return

        # Permission check
        if not real_guild.me.guild_permissions.create_instant_invite:
            await interaction.response.send_message(
                "<:undone:1398729502028333218> The bot doesn't have the 'Create Invite' permission.",
                ephemeral=True
            )
            return
        ...
        try:
            ...
            target_channel = None
            if real_guild.system_channel and real_guild.system_channel.permissions_for(real_guild.me).create_instant_invite:
                target_channel = real_guild.system_channel
            else:
                for channel in real_guild.text_channels:
                    if channel.permissions_for(real_guild.me).create_instant_invite:
                        target_channel = channel
                        break

            if not target_channel:
                await interaction.response.send_message(
                    "<:undone:1398729502028333218> No suitable channel found to create an invite.",
                    ephemeral=True
                )
                return
            ...
            invite = await target_channel.create_invite(
                ...
                max_uses=1,
                unique=True,
                reason=f"Generated by developer {interaction.user}"
            )

            await interaction.response.send_message(
                f"<:done:1398729525277229066> Invite for **{self.guild.name}**: {invite.url}",
                ephemeral=True
            )

        except Exception as e:
            await interaction.response.send_message(
                f"<:undone:1398729502028333218> An error occurred: {e}",
                ephemeral=True
            )
